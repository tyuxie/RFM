...
def rk3_step_func(func, t0, dt, t1, y0, reflect_fn, reflect_velocity_fn, butcher_tableu=None, f0=None, perturb=False):
    ...
    y1 = y0 + dt * k1 * butcher_tableu[1][1]
    refl_y1 = reflect_fn(y0, y1-y0)
    refl_k2 = func(t0 + dt * butcher_tableu[1][0], refl_y1)
    k2 = reflect_velocity_fn(y0, y1-y0, refl_k2)

    y2 = y0 + dt * (k1 * butcher_tableu[2][1] + k2 * butcher_tableu[2][2])
    refl_y2 = reflect_fn(y0, y2-y0)
    refl_k3 = func(t0 + dt * butcher_tableu[2][0], refl_y2)
    k3 = reflect_velocity_fn(y0, y2-y0, refl_k3)
...
class RKAdaptiveStepsizeODESolver(AdaptiveStepsizeEventODESolver):
    ...
    def __init__(self, func, y0, rtol, atol, reflect_fn, reflect_velocity_fn,
                 min_step=0,
                 max_step=float('inf'),
                 first_step=None,
                 step_t=None,
        ...
        self.reflect_velocity_fn = reflect_velocity_fn
    ...
    def _advance(self, next_t):
        ## XTY: we make a main change here.
        ## y0: start point of the last step
        ## y1: end point of the last step
        """Interpolate through the next time point, integrating as necessary."""
        n_steps = 0
        while next_t > self.rk_state.t1:
            if n_steps > 0:
                y1, f1, t0, t1, dt, interp_coeff = self.rk_state
                y1 = self.reflect_fn(y0, y1-y0)
                f1 = self.func(t1, y1)  ## we should calculate the velocity after reflecting it
                self.rk_state = _RungeKuttaState(y1, f1, t0, t1, dt, interp_coeff)
            else:
                y1 = self.rk_state.y1
            y0 = y1
        ...
        y1, f1, t0, t1, dt, interp_coeff = self.rk_state
        y_out = _interp_evaluate(self.rk_state.interp_coeff, self.rk_state.t0, self.rk_state.t1, next_t)
        y1 = self.reflect_fn(y0, y1-y0)
        f1 = self.func(t1, y1)
        self.rk_state = _RungeKuttaState(y1, f1, t0, t1, dt, interp_coeff)
            
        return self.reflect_fn(y0, y_out-y0)
    ...
    def _adaptive_step(self, rk_state):
        ...
        This function does not include reflect operation.
        """
        ...
        # y1 = self.reflect_fn(y0, y1-y0)         #XTY: we do not reflect y1 here
        ...
        if accept_step:
            ...
            f_next = f1   ##XTY: We do not reflect here.
        else:
    ...
    def _interp_fit(self, y0, y1, k, dt):
        ...
        # y_mid = self.reflect_fn(y_mid) #XTY: do not reflect
        f0 = k[..., 0]
        f1 = k[..., -1]
    ...
    def _runge_kutta_step(self, func, y0, f0, t0, dt, t1, tableau):
        ...
        for i, (alpha_i, beta_i) in enumerate(zip(tableau.alpha, tableau.beta)):
            ...
            refl_yi = self.reflect_fn(y0, yi-y0)
            relf_f = func(ti, refl_yi, perturb=perturb)
            f = self.reflect_velocity_fn(y0, yi-y0, relf_f)
